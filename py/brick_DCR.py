...
def create_brick_corrected_data(filename, corr_dir, old_dir):
    ...
    corr_table[1].columns.add_col(fits.Column(name="dcr_full_fit_x", array=np.zeros(len(corr_table[1].data)), format='E'))
    corr_table[1].columns.add_col(fits.Column(name="dcr_full_fit_y", array=np.zeros(len(corr_table[1].data)), format='E'))
    corr_table[1].columns.add_col(fits.Column(name="dcr_full_fit_dra", array=np.zeros(len(corr_table[1].data)), format='E'))
    corr_table[1].columns.add_col(fits.Column(name="dcr_full_fit_ddec", array=np.zeros(len(corr_table[1].data)), format='E'))
...
def create_corrected_data(corr_dir, old_dir, cont=True):
    for filename in os.listdir(old_dir):
        ...
        create_brick_corrected_data(finame, corr_dir, old_dir)


def create_corrected_gaia(corr_dir, old_dir, tractor_file, cont=True):
    for filename in os.listdir(old_dir):
        f = os.path.join(old_dir, filename)
        if not os.path.isfile(f) or filename[:4] != 'gaia' or filename[:6] == 'gaia-c' or (cont is True and filename in os.listdir(corr_dir)):
            continue
        print(filename)
